...
const props=defineProps({modelValue:Boolean,records:{type:Array,required:true},role:{type:String,required:true},roleMeta:{type:Object,required:true},stores:{type:Array,required:true},permissions:{type:Object,default:()=>({})}})
...
const canBatchImport = computed(() => canUseBatchImportInvitation(props.permissions, props.role === 'admin'))
function ensureBatchImportPermission(){
  if(canBatchImport.value)return true
  ElMessage.error('当前角色未配置批量导入邀约权限')
  return false
}

async function handleFile(event){
  if(!ensureBatchImportPermission())return
...
function createRecord(row,stamp,index){
  ...
  const staff={科臻澳总店:{service:'舒婷',butler:'林悦',director:'陈楠',manager:'赵阳'},金水形象店:{service:'舒婷',butler:'林悦',director:'陈楠',manager:'赵阳'},东区旗舰店:{service:'舒婷',butler:'林悦',director:'陈楠',manager:'赵阳'}}[row.store]||{}
  return {id,businessDate:row.date,appointmentTime:row.time,diagnosisType:row.diagnosisType,store:row.store,vip1:{name:row.customerName,phone:row.customerPhone},companions:row.companionName?[{name:row.companionName,phone:row.companionPhone}]:[],vip2:row.companionName?{name:row.companionName,phone:row.companionPhone}:null,cardConsultant:row.cardConsultant||'虚拟卡姐',beautyConsultant:row.beautyConsultant||'虚拟美导',estimatedProject:row.projects.join('、'),projects:row.projects,estimatedAmount:0,paymentType:'none',revenue:0,cardAmount:0,note:row.note,importSnapshot:{serial:row.serial,time:row.date+' '+row.time,diagnosisType:row.diagnosisType,cardConsultant:row.cardConsultant,beautyConsultant:row.beautyConsultant,market:row.market,store:row.store,vip1:row.customerName,vip1Phone:row.customerPhone,vip2:row.companionName,vip2Phone:row.companionPhone,remark:row.note,service:row.service},assignments:{market:row.market||'虚拟市场专员',service:row.service||'舒婷',butler:'林悦',director:'陈楠',manager:'赵阳',floorControl:'娜娜',consultant:'小咨',nurse:'张璐'},storeManager:'王晓歌',department:'',followupDate:addDays(row.date,60),status:'floorControl',appointmentStatus:'imported',flags:row.warnings.map(x=>`导入提醒：${x}`),source:'batch-import',floorControl:{createdTime:nowText(),operator:'娜娜'},doctorDiagnosis:{},serviceExecution:{operator:'舒婷'},followupRecords:[],logs:[{id:`${id}-created`,time:nowText(),operator:`${props.roleMeta.label}·${props.roleMeta.name}`,action:'批量导入邀约',detail:`导入后进入场控排诊；预约${row.date} ${row.time}到店`,fromStatus:'floorControl',toStatus:'floorControl',type:'primary'}]}
}
