...
const staff = reactive((saved?.staff || seedStaff()).map(normalizeStaff))
const projects = reactive(saved?.projects || seedProjects())
const roles = reactive((saved?.roles || seedRoles()).map(normalizeRole))
...
function persistSettings(){localStorage.setItem(SETTINGS_KEY,JSON.stringify({orgRegions,staff,projects,roles,performanceConfigs,storeRules,dictionaries,auditLogs}))}
...
function emitConfig(){const departmentNames=[...new Set(flatStores.value.flatMap(store=>(store.departments||[]).map(dept=>typeof dept==='string'?dept:dept.name)).filter(Boolean))];emit('config-change',{stores:flatStores.value.filter(x=>x.status==='active').map(x=>x.name),departments:departmentNames,projectCatalog:buildProjectCatalog()})}
...
function normalizeRole(row){const actionMap={'查看':'view','新增':'create','编辑':'edit','删除':'delete','导入':'import','导出':'export','库存操作':'operate'};const permissions=row.permissions?JSON.parse(JSON.stringify(row.permissions)):Object.fromEntries((row.pages||[]).map(page=>[page,(row.actions||[]).map(x=>actionMap[x]||x)]));if(row.key==='storeManager'&&!permissions.dailyReports)permissions.dailyReports=['view','create','edit','confirm'];if(['director','storeManager'].includes(row.key)&&!permissions.dealReports)permissions.dealReports=['view','export'];if(row.key==='admin'){permissionModules.forEach(module=>{permissions[module.key]=[...actionOptions]})}return {...row,permissions}}
...
function seedRoles(){const make=(id,key,label,dataScope,items)=>({id,key,label,dataScope,permissions:Object.fromEntries(items.map(([module,actions])=>[module,actions]))});return [make('role-market','market','市场','本人',[['workbench',['view','edit']],['customers',['view','create','edit']],['appointments',['view','create','edit']]]),make('role-service','service','客服','本店',[['workbench',['view','edit']],['customers',['view','edit']],['appointments',['view']]]),make('role-butler','butler','管家','本店',[['workbench',['view','edit']],['customers',['view']]]),make('role-director','director','总监','本店',[['workbench',['view','edit']],['customers',['view']],['dashboard',['view']]]),make('role-manager','storeManager','店长','本店',[['workbench',['view','create','edit']],['customers',['view','create','edit','export']],['appointments',['view','create','edit']],['dashboard',['view','export']],['dailyReports',['view','create','edit','confirm']],['org',['view','edit']],['staff',['view','create','edit','export']],['inventory',['view','create','edit','operate']],['rules',['view','edit']]]),make('role-admin','admin','平台管理员','全部门店',permissionModules.map(x=>[x.key,[...actionOptions]]))]}
