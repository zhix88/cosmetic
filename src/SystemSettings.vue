    ...
    <el-dialog v-model="entityVisible" :title="entityDialogTitle" :width="entityType === 'staff' || entityType === 'role' ? '900px' : '680px'">
      <el-form ref="entityFormRef" :model="entityForm" :rules="entityRules" label-position="top">
        ...
        <template v-else>
          ...
          <PermissionTree :catalog="permissionCatalog" :actions="permissionActions" :capabilities="permissionCapabilities" v-model="entityForm.permissions" />
        </template>
      </el-form>
...
const roleModuleIndex = modules.findIndex((item) => item.key === 'roles')
if (roleModuleIndex > 2) {
  const [roleModule] = modules.splice(roleModuleIndex, 1)
  modules.splice(2, 0, roleModule)
}
...
const permissionCapabilities = { workbench:['view','edit','operate'], customers:['view','create','edit','delete','import','export'], appointments:['view','create','edit','delete','import','export','confirm','return','operate'], dashboard:['view','export'], dailyReports:['view','export'], dealReports:['view','export'], org:['view','create','edit','delete'], staff:['view','create','edit','delete','import','export'], projects:['view','create','edit','delete','import','export'], packages:['view','create','edit','delete'], performance:['view','create','edit','delete','export'], inventory:['view','create','edit','delete','import','export','operate'], roles:['view','create','edit','delete'], rules:['view','edit'], dictionary:['view','create','edit','delete'], logs:['view','export'] }
const permissionCatalog = [
  { key:'workbench', label:'工作台', children:[
    { key:'progressBoard', label:'业务进度看板', children:['查看全部业务','查看本人待办','查看阶段卡片','查看业务详情','执行当前节点'].map((label,index)=>({key:`point${index}`,label,type:'point',actions:index===4?['view','operate']:['view']})) },
    { key:'batchImportInvitation', label:'批量导入邀约', children:['下载导入模板','上传导入文件','导入邀约记录','查看导入结果'].map((label,index)=>({key:`point${index}`,label,type:'point',actions:index===0||index===3?['view']:['view','operate']})) }
  ] },
  { key:'customers', label:'顾客档案', children:[
    { key:'customerList', label:'顾客列表', children:['查看','新增','编辑','启用/暂停','删除','导入','导出'].map((label,index)=>({key:`point${index}`,label,type:'point',actions:index===0?['view']:index===6?['view','export']:['view','operate']})) },
    { key:'customerDetail', label:'顾客详情', children:['查看基础资料','编辑人员归属','调整储值','调整积分','查看项目资产','购买项目/套餐','会员充值','查看服务记录','查看术前术后照片','查看/新增回访记录','上传回访照片','管理客户标签'].map((label,index)=>({key:`point${index}`,label,type:'point',actions:index===0||index===4||index===7||index===8?['view']:['view','operate']})) }
  ] },
  { key:'appointments', label:'预约记录', children:[
    { key:'appointmentList', label:'预约列表', children:['查看','新增','编辑','改期','取消预约','导入','导出'].map((label,index)=>({key:`point${index}`,label,type:'point',actions:index===6?['view','export']:['view','operate']})) },
    { key:'calendar', label:'日历排班', children:['查看预约','新增预约','修改预约'].map((label,index)=>({key:`point${index}`,label,type:'point',actions:index===0?['view']:['view','operate']})) }
  ] },
  { key:'dashboard', label:'缁忚惀鐪嬫澘', type:'report', actions:['view','export'], children:[] },
  { key:'dailyReports', label:'姣忔棩鎶ヨ〃', type:'report', actions:['view','export'], children:[] },
  { key:'dealReports', label:'鎴愪氦鎶ヨ〃', type:'report', actions:['view','export'], children:[] },
  ...[['org','组织架构',['组织、区域、门店、部门']],['staff','员工管理',['员工列表','员工详情']],['roles','角色权限',['角色列表','角色配置']],['projects','项目管理',['项目列表','项目配置','耗材模板']],['packages','活动套餐',['套餐列表','套餐项目明细']],['performance','业绩配置',['提成规则']],['inventory','耗材管理',['库存余额','批次效期','库存流水','预警中心']],['rules','业务规则',['预约容量规则','回访周期规则','会员积分规则','业务编号规则']],['dictionary','基础字典',['字典分类','字典选项']],['logs','操作日志',['审计日志']]].map(([key,label,children])=>({key,label,children:children.map((child,index)=>({key:`module${index}`,label:child,type:'module',actions:permissionCapabilities[key]||['view']}))}))
]
const permissionLabelMap = {
  workbench:'工作台', progressBoard:'业务进度看板', batchImportInvitation:'批量导入邀约',
  customers:'顾客档案', customerList:'顾客列表', customerDetail:'顾客详情', appointments:'预约记录', appointmentList:'预约列表', calendar:'日历排班',
  dashboard:'经营看板', dailyReports:'每日报表', dealReports:'成交报表', org:'组织架构', staff:'员工管理', roles:'角色权限', projects:'项目管理', packages:'活动套餐', performance:'业绩配置', inventory:'耗材管理', rules:'业务规则', dictionary:'基础字典', logs:'操作日志',
  point0:'查看', point1:'新增', point2:'编辑', point3:'改期', point4:'取消预约', point5:'导入', point6:'导出', module0:'经营概览', module1:'转化漏斗', module2:'业绩统计', dailyData:'日报数据', 'workbench.progressBoard.point0':'查看全部业务', 'workbench.progressBoard.point1':'查看本人待办', 'workbench.progressBoard.point2':'查看阶段卡片', 'workbench.progressBoard.point3':'查看业务详情', 'workbench.progressBoard.point4':'执行当前节点', 'workbench.batchImportInvitation.point0':'下载导入模板', 'workbench.batchImportInvitation.point1':'上传导入文件', 'workbench.batchImportInvitation.point2':'导入邀约记录', 'workbench.batchImportInvitation.point3':'查看导入结果', 'customers.customerList.point0':'查看', 'customers.customerList.point1':'新增', 'customers.customerList.point2':'编辑', 'customers.customerList.point3':'启用/暂停', 'customers.customerList.point4':'删除', 'customers.customerList.point5':'导入', 'customers.customerList.point6':'导出', 'customers.customerDetail.point0':'查看基础资料', 'customers.customerDetail.point1':'编辑人员归属', 'customers.customerDetail.point2':'调整储值', 'customers.customerDetail.point3':'调整积分', 'customers.customerDetail.point4':'查看项目资产', 'customers.customerDetail.point5':'购买项目/套餐', 'customers.customerDetail.point6':'会员充值', 'customers.customerDetail.point7':'查看服务记录', 'customers.customerDetail.point8':'查看术前术后照片', 'customers.customerDetail.point9':'查看/新增回访记录', 'customers.customerDetail.point10':'上传回访照片', 'customers.customerDetail.point11':'管理客户标签', 'appointments.appointmentList.point0':'查看', 'appointments.appointmentList.point1':'新增', 'appointments.appointmentList.point2':'编辑', 'appointments.appointmentList.point3':'改期', 'appointments.appointmentList.point4':'取消预约', 'appointments.appointmentList.point5':'导入', 'appointments.appointmentList.point6':'导出', 'appointments.calendar.point0':'查看预约', 'appointments.calendar.point1':'新增预约', 'appointments.calendar.point2':'修改预约'
}
function repairPermissionLabels(nodes, parent = '') {
  nodes.forEach((node) => {
    const path = parent ? `${parent}.${node.key}` : node.key
    if (permissionLabelMap[path]) node.label = permissionLabelMap[path]
    else if (permissionLabelMap[node.key] && !parent) node.label = permissionLabelMap[node.key]
    repairPermissionLabels(node.children || [], path)
  })
  return nodes
}
repairPermissionLabels(permissionCatalog)
const canonicalSectionLabels = {
  workbench: '工作台', customers: '顾客档案', appointments: '预约记录', dashboard: '经营看板', dailyReports: '每日报表', dealReports: '成交报表',
  org: '组织架构', staff: '员工管理', roles: '角色权限', projects: '项目管理', packages: '活动套餐', performance: '业绩配置', inventory: '耗材管理', rules: '业务规则', dictionary: '基础字典', logs: '操作日志'
}
permissionCatalog.forEach((node) => { if (canonicalSectionLabels[node.key]) node.label = canonicalSectionLabels[node.key] })
const canonicalFeatureLabels = {
  'workbench.progressBoard':'业务进度看板', 'workbench.batchImportInvitation':'批量导入邀约',
  'workbench.progressBoard.point0':'查看全部业务', 'workbench.progressBoard.point1':'查看本人业务', 'workbench.progressBoard.point2':'查看阶段卡片', 'workbench.progressBoard.point3':'查看任务详情', 'workbench.progressBoard.point4':'执行当前节点',
  'workbench.batchImportInvitation.point0':'下载邀约模板', 'workbench.batchImportInvitation.point1':'上传导入文件', 'workbench.batchImportInvitation.point2':'导入邀约记录', 'workbench.batchImportInvitation.point3':'查看导入结果',
  'customers.customerList':'顾客档案列表', 'customers.customerList.point0':'查看', 'customers.customerList.point1':'新增', 'customers.customerList.point2':'编辑', 'customers.customerList.point3':'启用/暂停', 'customers.customerList.point4':'删除', 'customers.customerList.point5':'导入', 'customers.customerList.point6':'导出',
  'customers.customerDetail':'顾客详情', 'customers.customerDetail.point0':'查看基础资料', 'customers.customerDetail.point1':'编辑人员归属', 'customers.customerDetail.point2':'调整储值', 'customers.customerDetail.point3':'调整积分', 'customers.customerDetail.point4':'查看项目资产', 'customers.customerDetail.point5':'购买项目/套餐', 'customers.customerDetail.point6':'会员充值', 'customers.customerDetail.point7':'查看服务记录', 'customers.customerDetail.point8':'查看术前术后照片', 'customers.customerDetail.point9':'查看/新增回访记录', 'customers.customerDetail.point10':'上传回访照片', 'customers.customerDetail.point11':'管理客户标签',
  'appointments.appointmentList':'预约列表', 'appointments.appointmentList.point0':'查看', 'appointments.appointmentList.point1':'新增', 'appointments.appointmentList.point2':'编辑', 'appointments.appointmentList.point3':'改期', 'appointments.appointmentList.point4':'取消预约', 'appointments.appointmentList.point5':'导入', 'appointments.appointmentList.point6':'导出',
  'appointments.calendar':'日历排班', 'appointments.calendar.point0':'查看预约', 'appointments.calendar.point1':'新增预约', 'appointments.calendar.point2':'修改预约'
}
const applyCanonicalFeatureLabels = (nodes, parent = '') => nodes.forEach((node) => { const path = parent ? `${parent}.${node.key}` : node.key; if (canonicalFeatureLabels[path]) node.label = canonicalFeatureLabels[path]; applyCanonicalFeatureLabels(node.children || [], path) })
applyCanonicalFeatureLabels(permissionCatalog)
...
function flattenPermissionCatalog(nodes, parent = '') {
  return nodes.flatMap((node) => { const path = parent ? `${parent}.${node.key}` : node.key; return [{ ...node, path }, ...flattenPermissionCatalog(node.children || [], path)] })
}
function normalizeTreePermissions(raw, roleKey = '') {
